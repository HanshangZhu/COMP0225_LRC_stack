...
class InitialPoseGuard(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("spawn_z", 0.38)
        self.declare_parameter("spawn_yaw", 0.0)
        self.declare_parameter("hold_sec", 14.0)
        self.declare_parameter("rate", 15.0)
        self.declare_parameter("request_timeout_sec", 0.8)
        self.declare_parameter("max_failures", 30)
        self.declare_parameter("retry_backoff_initial_sec", 0.1)
        self.declare_parameter("retry_backoff_max_sec", 1.5)
        ...
        self.hold_sec = max(0.5, float(self.get_parameter("hold_sec").value))
        self.rate = max(2.0, float(self.get_parameter("rate").value))
        self.request_timeout_sec = max(0.2, float(self.get_parameter("request_timeout_sec").value))
        self.max_failures = max(5, int(self.get_parameter("max_failures").value))
        self.retry_backoff_initial_sec = max(0.02, float(self.get_parameter("retry_backoff_initial_sec").value))
        self.retry_backoff_max_sec = max(
            self.retry_backoff_initial_sec,
            float(self.get_parameter("retry_backoff_max_sec").value),
        )
    ...
    def _on_set_state_done(self, future) -> None:
        ...
        if not result.success:
            ...
            self._next_try_wall = time.monotonic() + self._backoff_sec
            self._backoff_sec = min(self._backoff_sec * 1.8, self.retry_backoff_max_sec)
            self.get_logger().warn(f"SetEntityState success=false: {result.status_message}")
            return
