            ...
            if map_msg is not None:
                if now_ns - self._last_costmap_warn_ns > int(5e9):
                    ...
                    self.get_logger().warn(
                        f"Costmap {self.costmap_topic} unavailable/stale; falling back to raw map {self.map_topic}."
                    )
            ...
            if map_t > 0 and odom_t > 0:
                ...
                if dt > self.max_map_odom_dt:
                    if now.nanoseconds - self._last_sync_warn_ns > int(2e9):
                        self.get_logger().warn(
                            f"map/odom desync: dt={dt:.3f}s > {self.max_map_odom_dt:.3f}s; skipping frontier update"
                        )
