...
def _import_go2_to_stage(
    ...
    _enable_isaac_extensions()
    before_children = _world_children_paths(stage) if snapshot_before else set()
    ...
    root_path = ""
    if snapshot_before:
        after_children = _world_children_paths(stage)
        new_children = sorted(after_children - before_children)
        for path in new_children:
            lower = path.lower()
            if "go2" in lower or "description" in lower:
                root_path = path
                break
        if not root_path and new_children:
            root_path = new_children[0]
    ...
    if stage.GetPrimAtPath("/World/go2_description").IsValid():
        root_path = "/World/go2_description"
    ...
    if not stage.GetPrimAtPath(imported_path).IsValid():
        fallback_path = "/World/go2_description"
        if stage.GetPrimAtPath(fallback_path).IsValid():
    ...
    stale_path = "/World/go2_description"
