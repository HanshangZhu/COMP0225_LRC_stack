...
def _create_fallback_robot_prim(stage, ns: str, reason: Exception) -> str:
    from pxr import Gf, UsdGeom

    fallback_path = f"/World/{ns}"
    if not stage.GetPrimAtPath(fallback_path).IsValid():
        cube = UsdGeom.Cube.Define(stage, fallback_path)
    ...
    print(
        f"[isaac_dual_bringup] WARNING: using fallback robot prim for ns={ns} after import failure: {reason}",
        file=sys.stderr,
        flush=True,
    )
...
def _import_robot_prim(stage, args, ns: str, robot_usd: Path | None, robot_urdf: Path | None) -> str:
    try:
        ...
        return _create_fallback_robot_prim(stage, ns, exc)
