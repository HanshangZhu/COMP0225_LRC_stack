...
def generate_launch_description():
    ...
    contact_sensor = Node(
        ...
        condition=UnlessCondition(lite),
    ...
    # Register event handlers to load controllers AFTER spawn completes
    spawn_callback = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=start_gazebo_spawner_cmd,
            on_exit=[load_joint_state_controller, load_joint_trajectory_effort_controller],
        )
    )
    ...
    return LaunchDescription(
        [
            ...
            # Controllers are now loaded via callback
            spawn_callback,
            contact_sensor
        ]
    )
