...
bool ContourGraph::IsNavToGoalConnectFreePolygon(const NavNodePtr& node_ptr, const NavNodePtr& goal_ptr) {
    ...
    // DEBUG: log which check fails for odom node
    if (node_ptr->is_odom) {
        // Check each component separately
        int fail_at = 0;
        for (const auto& contour : ContourGraph::boundary_contour_) {
            if (!ContourGraph::IsEdgeOverlapInHeight(h_pair, HeightPair(contour.first, contour.second))) continue;
            if (ContourGraph::IsEdgeCollideSegment(contour, bd_cedge)) { fail_at = 1; break; }
        }
        if (!fail_at && !is_global_check) {
            int poly_idx = 0;
            for (const auto& poly_ptr : ContourGraph::contour_polygons_) {
                if (poly_ptr->is_pillar) { poly_idx++; continue; }
                const Point3D center_p((cedge.start_p.x + cedge.end_p.x)/2.f, (cedge.start_p.y + cedge.end_p.y)/2.f, 0.f);
                bool inside_now = FARUtil::PointInsideAPoly(poly_ptr->vertices, center_p);
                bool collide = ContourGraph::IsEdgeCollidePoly(poly_ptr->vertices, cedge);
                if ((poly_ptr->is_robot_inside != inside_now) || collide)
                {
                    fail_at = 2;
                    // Log details about the blocking polygon
                    std::cout << "[FreePoly BLOCK] poly_idx=" << poly_idx
                              << " nverts=" << poly_ptr->vertices.size()
                              << " is_robot_inside=" << poly_ptr->is_robot_inside
                              << " inside_now=" << inside_now
                              << " collide=" << collide
                              << " edge=(" << cedge.start_p.x << "," << cedge.start_p.y
                              << ")->(" << cedge.end_p.x << "," << cedge.end_p.y << ")"
                              << " center=(" << center_p.x << "," << center_p.y << ")" << std::endl;
                    // Print first few polygon vertices
                    int nv = std::min((int)poly_ptr->vertices.size(), 8);
                    std::cout << "  verts:";
                    for (int vi = 0; vi < nv; vi++) {
                        std::cout << " (" << poly_ptr->vertices[vi].x << "," << poly_ptr->vertices[vi].y << ")";
                    }
                    if ((int)poly_ptr->vertices.size() > nv) std::cout << " ...+" << (poly_ptr->vertices.size()-nv) << " more";
                    std::cout << std::endl;
                    break;
                }
                poly_idx++;
            ...
            if (!fail_at) {
                for (const auto& c : ContourGraph::unmatched_contour_) {
                    if (ContourGraph::IsEdgeCollideSegment(c, cedge)) { fail_at = 3; break; }
                }
            }
            if (!fail_at) {
                for (const auto& c : ContourGraph::inactive_contour_) {
                    if (ContourGraph::IsEdgeCollideSegment(c, cedge)) { fail_at = 4; break; }
                }
            }
        } else if (!fail_at) {
            for (const auto& c : ContourGraph::global_contour_) {
                if (!ContourGraph::IsEdgeOverlapInHeight(h_pair, HeightPair(c.first, c.second))) continue;
                if (ContourGraph::IsEdgeCollideSegment(c, cedge)) { fail_at = 5; break; }
            }
            if (!fail_at) {
                for (const auto& poly_ptr : ContourGraph::contour_polygons_) {
                    if (poly_ptr->is_pillar) continue;
                    if (ContourGraph::IsEdgeCollidePoly(poly_ptr->vertices, cedge)) { fail_at = 6; break; }
                }
            }
        }
        static int log_ctr = 0;
        if (++log_ctr % 10 == 1) {
            std::cout << "[FreePoly] global=" << is_global_check
                      << " boundary=" << ContourGraph::boundary_contour_.size()
                      << " polys=" << ContourGraph::contour_polygons_.size()
                      << " unmatched=" << ContourGraph::unmatched_contour_.size()
                      << " inactive=" << ContourGraph::inactive_contour_.size()
                      << " global_c=" << ContourGraph::global_contour_.size()
                      << " fail_at=" << fail_at << std::endl;
        }
    }
