#!/usr/bin/env python3
"""Single Go2W VLM-in-the-loop Gazebo exploration launch for the FAR backend.
...
Usage:
  ...
  ros2 launch vlm_explorer single_vlm_gazebo_far.launch.py vlm_model:=gpt-4o-mini
"""
...
def _launch_setup(context):
    ...
    if slam_source != "cartographer" and map_backend != "scan":
        raise ValueError(
    ...
    # ── 1. Base launch: Gazebo + Go2W + CFPA2 ────────────────────────
    ...
    base_args = {
        ...
        "broadcast_tf": "false" if slam_source == "cartographer" else "true",
    ...
    if _as_bool(rviz):
        rviz_config = os.path.join(vlm_pkg, "rviz", "single_vlm_gazebo_far.rviz")
        actions.append(
            TimerAction(
                period=7.0,
                actions=[
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                ...
                default_value="far",
                description="Navigation execution backend: far, reactive, or mppi",
            ),
            DeclareLaunchArgument(
                "frontier_backend",
