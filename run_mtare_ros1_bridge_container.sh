...
compute_bridge_rev() {
  local dockerfile_path="${WORKSPACE_DIR}/docker/mtare_ros1_bridge/Dockerfile"
  local stack_script_path="${WORKSPACE_DIR}/docker/mtare_ros1_bridge/run_mtare_ros1_bridge_stack.sh"
...
if [[ ! -f "${WORKSPACE_DIR}/docker/mtare_ros1_bridge/Dockerfile" ]]; then
  echo "Dockerfile not found at ${WORKSPACE_DIR}/docker/mtare_ros1_bridge/Dockerfile" >&2
  exit 1
fi
if [[ ! -f "${WORKSPACE_DIR}/docker/mtare_ros1_bridge/run_mtare_ros1_bridge_stack.sh" ]]; then
  echo "Container stack script not found at ${WORKSPACE_DIR}/docker/mtare_ros1_bridge/run_mtare_ros1_bridge_stack.sh" >&2
  exit 1
fi
...
if [[ "${need_build}" == "1" ]]; then
  DOCKER_BUILD_ARGS=(
    ...
    -f "${WORKSPACE_DIR}/docker/mtare_ros1_bridge/Dockerfile"
  )
  if [[ "${NO_CACHE}" == "1" ]]; then
